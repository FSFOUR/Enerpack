...
const CardFace: React.FC<{ 
  ...
  const labelWidth = isPrintView ? "w-[42mm]" : "w-[110px] md:w-[140px]";
  
  const rowHeight = isPrintView ? "h-[14mm]" : "h-11 md:h-14";
  const sigHeight = isPrintView ? "h-[34mm]" : "h-20 md:h-24";
  const labelTextSize = isPrintView ? "text-[10px]" : "text-[9px] md:text-[11px]";
  const valueTextSize = isPrintView ? "text-[13px]" : "text-sm md:text-[15px]";
    ...
    <div className={`bg-white border-[2pt] md:border-[3pt] ${borderStyle} flex flex-col box-border overflow-hidden ${cardWidth} ${cardHeight} ${isPrintView ? 'm-0 print:border-black' : 'shadow-xl mb-6 md:mb-12 mx-auto'}`}>
              ...
              {isPrintView ? (
                ...
                f.key === 'deliveryLocation' ? (
                  <div className="flex gap-1 w-full overflow-x-auto scrollbar-hide py-1">
                    <input 
                      className="flex-1 min-w-[60px] bg-transparent border-none outline-none font-black uppercase text-black text-sm md:text-base placeholder:text-slate-200"
                      value={card[f.key] || ''}
                    ...
                    <div className="flex gap-1">
                      {locations.map(loc => (
                        <button 
                          ...
                          className={`px-2 py-0.5 text-[8px] font-black rounded border ${card[f.key] === loc ? 'bg-black text-white border-black' : 'bg-slate-100 text-slate-500 border-slate-200'}`}
                        >
                          {loc}
                        </button>
                      ))}
                  ...
                  <input 
                    className="w-full bg-transparent border-none outline-none font-black uppercase text-black text-sm md:text-base placeholder:text-slate-200"
                    value={card[f.key] || ''}
                    onChange={(e) => updateCard?.(card.id, f.key, e.target.value)}
                    placeholder="---"
                  />
      ...
      <div className={`flex ${sigHeight} items-stretch overflow-hidden`}>
        <div className={`flex-1 border-r-[1.5pt] md:border-r-[2pt] ${borderStyle} flex flex-col print:border-black`}>
          <div className={`h-[8mm] md:h-[10mm] border-b-[1pt] md:border-b-[1.5pt] ${borderStyle} flex items-center px-2 md:px-4 bg-slate-50 print:bg-slate-50 print:border-black`}>
            <span className="text-[8px] md:text-[9px] font-black uppercase text-black">SUPERVISOR</span>
          </div>
          <div className="flex-1 bg-white"></div>
        </div>
        <div className="flex-1 flex flex-col">
          <div className={`h-[8mm] md:h-[10mm] border-b-[1pt] md:border-b-[1.5pt] ${borderStyle} flex items-center px-2 md:px-4 bg-slate-50 print:bg-slate-50 print:border-black`}>
            <span className="text-[8px] md:text-[9px] font-black uppercase text-black">ACCOUNTANT</span>
          </div>
          <div className="flex-1 bg-white"></div>
        </div>
      </div>
        ...
        <aside className="w-full lg:w-[400px] border-b lg:border-b-0 lg:border-r border-slate-200 bg-white flex flex-col relative z-20 shrink-0 shadow-xl overflow-y-auto">
          ...
          <div className="p-4 lg:p-6 space-y-4">
             <div className="space-y-2">
                ...
                <textarea 
                  className="w-full h-32 lg:h-80 p-4 rounded-xl border border-slate-200 resize-none outline-none text-xs font-mono bg-slate-50 focus:bg-white focus:border-blue-500 transition-all shadow-inner"
