...
import { ArrowLeft, Clock, Search, Truck, Download, Loader2, Pencil, X, Save } from 'lucide-react';
...
const STATUS_OPTIONS = ["Cutting", "Cutting Finished", "Out of Stock", "Order Placed", "Waiting for Reel", "Pending", "Delivered", "Cancelled", "Other"];
...
const EditWorkModal: React.FC<{
  ...
  onClose: () => void;
  onSave: (id: string, updates: Partial<StockTransaction>) => void;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        <div className="bg-[#0c4a6e] text-white p-6 font-black uppercase tracking-widest flex items-center justify-between">
          <div className="flex items-center gap-3"><Pencil className="w-5 h-5" /> Edit Pending Work</div>
          <button onClick={onClose} className="p-1 hover:bg-white/20 rounded-full transition-all"><X className="w-6 h-6" /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-8 space-y-4 overflow-y-auto scrollbar-hide">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.date} onChange={e => setFormData({ ...formData, date: e.target.value })} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Company</label>
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold uppercase" value={formData.company} onChange={e => setFormData({ ...formData, company: e.target.value.toUpperCase() })} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Size</label>
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.size} onChange={e => setFormData({ ...formData, size: e.target.value })} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">GSM</label>
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.gsm} onChange={e => setFormData({ ...formData, gsm: e.target.value })} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Item Code</label>
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold uppercase" value={formData.itemCode} onChange={e => setFormData({ ...formData, itemCode: e.target.value.toUpperCase() })} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Quantity</label>
              <input type="number" step="any" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.quantity} onChange={e => setFormData({ ...formData, quantity: Number(e.target.value) })} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Unit</label>
              <select className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold uppercase" value={formData.unit} onChange={e => setFormData({ ...formData, unit: e.target.value })}>
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Sheets (Override)</label>
              <input type="number" step="any" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.sheets || ''} onChange={e => setFormData({ ...formData, sheets: e.target.value === '' ? undefined : Number(e.target.value) })} />
            </div>
          </div>
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Work Name</label>
            <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.workName} onChange={e => setFormData({ ...formData, workName: e.target.value })} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Cut Size</label>
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.cuttingSize} onChange={e => setFormData({ ...formData, cuttingSize: e.target.value })} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 ml-1">Remarks</label>
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm font-bold" value={formData.remarks} onChange={e => setFormData({ ...formData, remarks: e.target.value })} />
            </div>
          </div>
          <div className="flex gap-4 pt-4">
            <button type="submit" className="flex-1 py-4 bg-[#0c4a6e] text-white rounded-2xl font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all">Update Record</button>
            <button type="button" onClick={onClose} className="px-8 py-4 text-slate-400 hover:text-slate-600 font-bold uppercase text-[10px] tracking-widest">Cancel</button>
          </div>
        </form>
      </div>
    </div>
...
const PendingWorks: React.FC<PendingWorksProps> = ({ transactions, onBack, onUpdateTransaction, onUpdatePriority, isAdmin }) => {
  ...
  const [editWork, setEditWork] = useState<StockTransaction | null>(null);
    ...
    <div className="flex flex-col h-full bg-[#f8fafc] relative">
      ...
      <div className="p-4 border-b flex flex-col md:flex-row justify-between items-start md:items-center shadow-sm bg-white gap-3 md:gap-0 no-print">
        <div className="flex items-center gap-4 w-full md:w-auto px-4">
            ...
            <h2 className="text-lg md:text-xl font-black text-[#0c4a6e] flex items-center gap-2 border-l border-slate-100 pl-4 ml-2 uppercase tracking-tighter"><Clock className="w-6 h-6 text-orange-600" />PENDING WORKS</h2>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto px-4">
             <div className="relative flex-1 md:flex-none">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-300" />
                <input type="text" placeholder="Search operational queue..." className="pl-10 pr-4 py-2 rounded-2xl border border-slate-100 text-sm w-full md:w-72 focus:outline-none focus:ring-4 focus:ring-[#0c4a6e]/10 bg-slate-50 text-center font-black" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
      ...
      <div ref={pendingContainerRef} className="flex-1 overflow-auto p-6">
        <div className="overflow-x-auto rounded-[2.5rem] border border-slate-200 shadow-xl bg-white mobile-bottom-scroll">
            <div className="min-w-[1250px] p-4">
                <table className="w-full text-xs text-center border-collapse">
                    <thead className="bg-orange-600 text-white sticky top-0 z-10 rounded-t-[2rem]">
                        <tr>
                            ...
                            <th className="p-3 border border-white/10 whitespace-nowrap uppercase tracking-widest">REMARKS</th>
                            <th className="p-3 border border-white/10 last:rounded-tr-[1.5rem] whitespace-nowrap uppercase tracking-widest">ACTIONS</th>
                        </tr>
                    </thead>
                        ...
                        {displayedTransactions.map(t => (
                            <tr 
                              ...
                              className={`transition-colors font-black ${
                                t.status === 'Cancelled' 
                                  ? 'bg-rose-100/70 text-slate-400 opacity-80' 
                                  : 'odd:bg-white even:bg-orange-50/30 hover:bg-amber-50'
                              }`}
                            >
                                <td className="p-3 border border-gray-100">{t.date}</td>
                                <td className="p-3 border border-gray-100">{t.size}</td>
                                <td className="p-3 border border-gray-100">{t.gsm}</td>
                                <td className={`p-3 border border-gray-100 ${t.status === 'Cancelled' ? '' : 'text-rose-600'}`}>
                                  {t.quantity} <span className="text-[9px] font-normal uppercase">{t.unit}</span>
                                </td>
                                <td className="p-3 border border-gray-100 uppercase text-[10px]">{t.company}</td>
                                <td className={`p-3 border border-gray-100 uppercase ${t.status === 'Cancelled' ? '' : 'text-indigo-600'}`}>{t.itemCode || '-'}</td>
                                <td className="p-3 border border-gray-100 min-w-[300px] max-w-[450px] truncate text-left px-5" title={t.workName}>{t.workName}</td>
                                <td className={`p-3 border border-gray-100 ${t.status === 'Cancelled' ? '' : 'text-indigo-600'}`}>{t.cuttingSize}</td>
                                <td className={`p-3 border border-gray-100 ${t.status === 'Cancelled' ? 'bg-rose-50/30' : 'text-blue-700 bg-blue-50/50'}`}>
                                ...
                                <td className="p-3 border border-gray-100">
                                   <div className="flex justify-center">
                                      {isAdmin ? (
                                        <button 
                                          onClick={() => setEditWork(t)}
                                          className="p-2 bg-slate-50 text-[#0c4a6e] hover:bg-[#0c4a6e] hover:text-white rounded-xl transition-all border border-slate-100 active:scale-90"
                                          title="Edit Entry"
                                        >
